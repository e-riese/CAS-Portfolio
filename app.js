...
function closeAllPopups() {
    ...
    activePopups.forEach(popup => {
        // 1. Stop Videos (Your existing code)
        const videos = popup.querySelectorAll('video');
        videos.forEach(v => v.pause());

        // 2. STOP SOUNDCLOUD (The Fix)
        const iframes = popup.querySelectorAll('iframe');
        iframes.forEach(iframe => {
            const currentSrc = iframe.src;
            iframe.src = '';         // Clear it to stop the music
            iframe.src = currentSrc; // Put it back so it's ready for the next open
        });
